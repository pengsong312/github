...
<html lang="zh-CN">
	...
	function checkAppIntf(str){
		$("#spaName").html("");
		var appIntf = $("#appIntf").val();//id为username的文本框的值
		if(str!=appIntf){
			$.ajax(  
              {  
                   type:"post",  
                   url:"mobile/checkAppIntf.do",  
                   data:"appIntf="+$("#appIntf").val(),  
                   dataType:"json",  
                   success:function(data)  
                    {  
                        ...
                        {       
                        	$("#spaName").html("<font color=red>不可为空</font>");  
                            $("#appIntf").focus(); 
                           	return false;                
			...
			<div class="panel-heading">
				<div>
					<span><img src="material/images/up.png" alt="" /></span>
					 APP修改
					<a href="mobile/findAllApp.do" class='back pull-right'>返回首页</a>
				</div>
			</div>
			
			<div class="panel-body">
				<form action="mobile/updateApp.do" class="form-horizontal" id="appForm" name ="appForm" enctype="multipart/form-data" method="post">
					<div class="form-group">
					    <button class="btn btn-upload" type="button" onclick="window.location=('/spring_mybatis/mobile/chooseApp.do')">返回</button>
					</div> 
					
					<input type="hidden" name="appNumber" value="${updateApp.appNumber}"/>
					<input type="hidden" name="picture" value="${updateApp.picture}"/>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					    	<input type="text" class="form-control" placeholder="" name="appIntf" id="appIntf"  onblur="checkAppIntf('${updateApp.appIntf}')" value="${updateApp.appIntf}">
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					      	...
					      	<select class="form-control" placeholder="" name="style" id="style" onclick="delStyle()">
					      		<option selected="selected" value="${updateApp.style}">${updateApp.style}</option>
					...
					<div class="form-group">
					    ...
					    <!--<input type="text" class="form-control" placeholder="" name="industry" id="industry">-->
					      	<select class="form-control" placeholder="" name="industry" id="industry" onclick="delIndustry()">
					      		<option selected="selected" value="${updateApp.industry}">${updateApp.industry}</option>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					    	<select class="form-control" placeholder="" name="color" id="color" onclick="delColor()">
					      	<c:choose>
					    		<c:when test="${updateApp.color=='white'}">
					    			<option value="${updateApp.color}" selected="selected">白</option>
					    		</c:when>
					    		<c:when test="${updateApp.color=='black'}">
					    			<option value="${updateApp.color}" selected="selected">黑</option>
					    		</c:when>
					    		<c:when test="${updateApp.color=='green'}">
					    			<option value="${updateApp.color}" selected="selected">绿</option>
					    		</c:when>
					    		<c:when test="${updateApp.color=='red'}">
					    			<option value="${updateApp.color}" selected="selected">红</option>
					    		</c:when>
					    		<c:when test="${updateApp.color=='blue'}">
					    			<option value="${updateApp.color}" selected="selected">蓝</option>
					    		</c:when>
					    		<c:when test="${updateApp.color=='orange'}">
					    			<option value="${updateApp.color}" selected="selected">橙</option>
					    		</c:when>
					    		<c:when test="${updateApp.color=='purple'}">
					    			<option value="${updateApp.color}" selected="selected">紫</option>
					    		</c:when>					    		
					    		<c:when test="${updateApp.color=='yellow'}">
					    			<option value="${updateApp.color}" selected="selected">黄</option>
					    		</c:when>					    		
					    		<c:when test="${updateApp.color=='gray'}">
					    			<option value="${updateApp.color}" selected="selected">其他</option>
					    		</c:when>
					    	</c:choose>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					      	...
					      	<select class="form-control" placeholder="" name="designer" id="designer" onclick="delDesigner()">
					      		<option selected="selected" value="${updateApp.designer}">${updateApp.designer}</option>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					      	<input type="text" class="form-control" placeholder="" name="appUrl" id="appUrl" value="${updateApp.appUrl}">
					    </div>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-8">
					      	<input type="text" class="form-control" placeholder="" name="describe" id="describe" value="${updateApp.describe}">
					    </div>
					</div>
