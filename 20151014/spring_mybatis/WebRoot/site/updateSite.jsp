...
<html lang="zh-CN">
	...
	function checkSiteIntf(str){
		$("#spaName").html("");
		var siteIntf = $("#siteIntf").val();//id为username的文本框的值
		if(str==siteIntf){
			return false;
		}else{
			$.ajax(  
              {  
                   type:"post",  
                   url:"site/checkSiteIntf.do",  
                   data:"siteIntf="+$("#siteIntf").val(),  
                   dataType:"json",  
	...
	function sub(){
		...
		if(span=='不可使用'){
			alert("请重新修改编号！");
			document.getElementById("siteIntf").focus();
			return false;
		}else{
			if(file!=""){
				if(confirm("即将覆盖全部图片，是否继续？")){
					if(subForm()){
	...
	function delIndustry(){
   		var $select = $("#industry");//designer下拉框Id
		var $options = $select.find("option");
		for(var i=0,max=$options.length;i<max;i++){
    		var $item1 = $options.eq(i);
    		for(var j=0;j<max;j++){
	...
	<div class="container " id="panel-wrapper">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form action="site/updateSite.do" class="form-horizontal" id="siteForm" name ="siteForm" enctype="multipart/form-data" method="post">
					<div class="form-group">
					    <button class="btn btn-upload" type="button" onclick="window.location=('/spring_mybatis/site/chooseSite.do')">返回</button>
					</div> 
					
					<input type="hidden" name="siteNumber" value="${updateSite.siteNumber}"/>
					<input type="hidden" name="picture" value="${updateSite.picture}"/>
					<div class="form-group">
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					    	<input type="text" class="form-control" placeholder="" name="siteIntf" id="siteIntf"  onblur="checkSiteIntf('${updateSite.siteIntf}')" value="${updateSite.siteIntf}">
					...
					<div class="form-group">
					    <label class="col-lg-1 col-lg-offset-2 control-label">编号：</label>
					    <div class=" col-lg-5">
					      	<input type="text" class="form-control" placeholder="" name="siteIntf" id="siteIntf" value="${updateSite.siteIntf}"> 					 
					    </div>
					</div>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					      	...
					      	<select class="form-control" placeholder="" name="style" id="style" onclick="delStyle()">
					      		<option selected="selected" value="${updateSite.style}">${updateSite.style}</option>
					...
					<div class="form-group">
					    ...
					    <!--<input type="text" class="form-control" placeholder="" name="industry" id="industry">-->
					      	<select class="form-control" placeholder="" name="industry" id="industry" onclick="delIndustry()">
					      		<option selected="selected" value="${updateSite.industry}">${updateSite.industry}</option>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					    	<select class="form-control" placeholder="" name="color" id="color" onclick="delColor()">					    	
					      	<c:choose>
					    		<c:when test="${updateSite.color=='white'}">
					    			<option value="${updateSite.color}" selected="selected">白</option>
					    		</c:when>
					    		<c:when test="${updateSite.color=='black'}">
					    			<option value="${updateSite.color}" selected="selected">黑</option>
					    		</c:when>
					    		<c:when test="${updateSite.color=='green'}">
					    			<option value="${updateSite.color}" selected="selected">绿</option>
					    		</c:when>
					    		<c:when test="${updateSite.color=='red'}">
					    			<option value="${updateSite.color}" selected="selected">红</option>
					    		</c:when>
					    		<c:when test="${updateSite.color=='blue'}">
					    			<option value="${updateSite.color}" selected="selected">蓝</option>
					    		</c:when>
					    		<c:when test="${updateSite.color=='orange'}">
					    			<option value="${updateSite.color}" selected="selected">橙</option>
					    		</c:when>
					    		<c:when test="${updateSite.color=='purple'}">
					    			<option value="${updateSite.color}" selected="selected">紫</option>
					    		</c:when>					    		
					    		<c:when test="${updateSite.color=='yellow'}">
					    			<option value="${updateSite.color}" selected="selected">黄</option>
					    		</c:when>					    		
					    		<c:when test="${updateSite.color=='gray'}">
					    			<option value="${updateSite.color}" selected="selected">其他</option>
					    		</c:when>
					    	</c:choose>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					      	...
					      	<select class="form-control" placeholder="" name="designer" id="designer" onclick="delDesigner()">
					      		<option selected="selected" value="${updateSite.designer}">${updateSite.designer}</option>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-5">
					      	<input type="text" class="form-control" placeholder="" name="siteUrl" id="siteUrl" value="${updateSite.siteUrl}">
					    </div>
					...
					<div class="form-group">
					    ...
					    <div class=" col-lg-8">
					      	<input type="text" class="form-control" placeholder="" name="describe" id="describe" value="${updateSite.describe}">
					    </div>
					</div>
